...
import { CoursesService, Formation, StatutFormation, TypeFormation } from '../../../../services/courses.service';
...
@Component({
  ...
  styleUrls: ['./formationform.component.scss']
})
export class FormationformComponent implements OnInit {
  ...
  loadFormationData(id: number): void {
    this.coursesService.getFormationById(id).subscribe({
      next: (formation) => {
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode && this.formationId) {
      ...
      this.coursesService.updateFormation(this.formationId, formationData, this.selectedFile).subscribe({
        next: () => {
          alert('Formation mise à jour avec succès');
          this.router.navigate(['/agent/formations']);
        },
        error: (err) => {
      ...
      this.coursesService.createFormation(formationData, this.selectedFile).subscribe({
        next: () => {
