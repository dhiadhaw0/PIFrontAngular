...
export class BookingsComponent implements OnInit {
  ...
  getFilteredFormations(): Formation[] {
    const statusMap = {
      'available': StatutFormation.En_Cours,
      'in_progress': StatutFormation.En_Cours,
      'completed': StatutFormation.Complète
    };
  ...
  enrollInCourse(formationId: number): void {
    ...
    this.formations = this.formations.map(formation => {
      if (formation.idFormation === formationId) {
        return { ...formation, statutFormation: StatutFormation.En_Cours };
      }
      return formation;
    });
  }
